    ...
    public CoinData fetchData() {
        try {
            ...
            for (Element element : buyElements) {
                Trade trade = new Trade(Double.valueOf(element.child(1).text().substring(this.target.getPlaceholderLength())),
                                                Double.valueOf(element.child(2).text().substring(this.target.getPlaceholderLength())));
                buyList.add(trade);
            }
            Elements sellElements = doc.select(this.target.getSellOutSelector());
            List<Trade> sellList = new ArrayList<Trade>();
            for (Element element : sellElements) {
                Trade trade = new Trade(Double.valueOf(element.child(1).text().substring(this.target.getPlaceholderLength())),
                                        Double.valueOf(element.child(2).text().substring(this.target.getPlaceholderLength())));
                logger.info(element.child(1).text().substring(this.target.getPlaceholderLength()) + " " 
                                        + element.child(2).text().substring(this.target.getPlaceholderLength()));
                sellList.add(trade);
            }
            data.setBuy(buyList);
