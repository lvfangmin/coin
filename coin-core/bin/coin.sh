...
usage() {
    cat <<EOF
Usage: coin.sh (start|stop|restart) <args...>

where argument is one of:
    -force (accepted only with stop command): Decides whether to stop the Coin Server forcefully if not stopped by normal shutdown
EOF
}
...
COIN_PID_FILE=$COIN_PID_DIR/coin.pid
...
LOG_SETTING="-Dlog4j.configuration=log4j.properties -Dcoin.log.dir=$COIN_LOG_DIR -Dcoin.log.file=coin.log -Dcoin.log.level=$COIN_LOG_LEVEL -Dcoin.log.appender=$COIN_LOG_APPENDER"
    ...
    start)
        if [ -f $COIN_PID_FILE ]; then
            if kill -0 `cat $COIN_PID_FILE` > /dev/null 2>&1; then
                echo "Coin server already running as process `cat $COIN_PID_FILE`."
                exit 1
            fi
        fi
        ...
        then
            ...
            then
                sleep 1
                echo "Successfully started coin server"
            else
                echo "Failed to write coin server pid"
                exit 1
            fi
        else
            echo "Failed to start coin server"
            exit 1
        fi
        ;;
    stop)
        if [ -f $COIN_PID_FILE ]; then
            ...
            if kill -0 $TARGET_PID > /dev/null 2>&1; then
                echo "Shutting down coin server..."
                kill $TARGET_PID
                count=0
                ...
                if kill -0 $TARGET_PID > /dev/null 2>&1; then
                    ...
                    if [ "$1" == "-force" ]
                    then
                        echo "Forcefully stopping coin server"
                        kill -9 $TARGET_PID >/dev/null 2>&1
                        echo "Successfully stopped the process"
                    else
                        echo "WARNNING: Coin Server is not stopped completely."
                        exit 1
                    fi
                fi
            else
                echo "No coin server to stop"
            fi
            rm $COIN_PID_FILE
        else
            echo "No coin server to stop"
